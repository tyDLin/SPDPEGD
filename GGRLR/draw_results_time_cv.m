...
colors  = {'k','y','b','g','m','c','c','r','r','k'};
markers = {'^','+','o','+','o','>','<','h','o','v','p','h'};
LStyles = {'-','-','-','-','-','-','-','-','-'};
funfcn_batch = {@EGADM};
...
datasets     = {'svmguide3','mushrooms','a9a','w8a'};
...
for idx_dataset = 1:length(datasets)
    ...
    for idx_method = 1:length(funfcns)
        stoc_data = load(['results_GGRLR_' func2str(funfcns{idx_method}) '_' dataset_name '.mat'],'trace_passes','trace_time','trace_accuracy','trace_obj_val','trace_test_loss');
...
if en_subplot
    ...
    suptitle('The Comparsion of All Methods on Graph-Guided Regularized Logistic Regression');
